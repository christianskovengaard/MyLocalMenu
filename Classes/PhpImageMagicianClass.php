...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
    ...
    // Flip image
    public function flip($way)
    {
        # Author:   Benjamin S L
        # Date:     04-03-14

        if($way==1 || $way=="vertical"){
            imageflip($this->imageResized, IMG_FLIP_HORIZONTAL);
        }elseif ($way == 2 || $way == "horizontal") {
            imageflip($this->imageResized, IMG_FLIP_VERTICAL);
        }elseif ($way == 2 || $way == "both") {
            imageflip($this->imageResized, IMG_FLIP_BOTH);
        }
    ...
    public function resizeToWidth($new_width){
        ...
        $this->resizeImage($new_width, $new_height);
    }


    // Resize to height
    public function resizeToHeight($new_height){
        # Author:   Benjamin S L
        # Date:     25-03-14
    ...
    public function showImage()
    {
        ...
        switch ($this->fileExtension) {
            ...
            default:
                // *** No extension - No save.
                $this->errorArray[] = 'This file type (' . $this->fileExtension . ') is not supported. File not saved.';
                break;
        }
